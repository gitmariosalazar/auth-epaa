 ...
 */
@Injectable()
export class PostgreSQLVerificationPersistence implements IVerificationRepository {
  private readonly logger = new Logger(PostgreSQLVerificationPersistence.name);
  ...
  private mapRowToModel(row: any): VerificationModel {
    ...
    model.fechaVerificado   = row.fecha_verificado ? new Date(row.fecha_verificado) : null;
