...
@Injectable()
export class PostgreSQLUserEmployeePersistence
  implements InterfaceUserEmployeeRepository
{
  ...
  async findById(employeeId: string): Promise<UserEmployeeResponse | null> {
    ...
    const result = await this.databaseService.query<UserEmployeeSQLResult>(query, [employeeId]);
    return result.length > 0 ? UserEmployeeAdapter.fromUserEmployeeSQLResultToUserEmployeeSQLResponse(result[0]) : null;
  }
  ...
  async findByUserId(userId: string): Promise<UserEmployeeResponse | null> {
    ...
    const result = await this.databaseService.query<UserEmployeeSQLResult>(query, [userId]);
    return result.length > 0 ? UserEmployeeAdapter.fromUserEmployeeSQLResultToUserEmployeeSQLResponse(result[0]) : null;
  }
  ...
  async findByIdCard(idCard: string): Promise<UserEmployeeResponse | null> {
    ...
    const result = await this.databaseService.query<UserEmployeeSQLResult>(query, [idCard]);
    return result.length > 0 ? UserEmployeeAdapter.fromUserEmployeeSQLResultToUserEmployeeSQLResponse(result[0]) : null;
  }
  ...
  async searchByName(searchTerm: string): Promise<UserEmployeeResponse[]> {
    ...
    const result = await this.databaseService.query<UserEmployeeSQLResult>(query, [`%${searchTerm}%`]);
    return result.map(row => UserEmployeeAdapter.fromUserEmployeeSQLResultToUserEmployeeSQLResponse(row));
  }
  ...
  async create(employee: UserEmployeeModel): Promise<UserEmployeeResponse> {
    return await this.databaseService.transaction(async (client: IDatabaseClient) => {
      ...
      if (employee.citizenId) {
        const { affectedRows: rowCount } = await client.execute('SELECT 1 FROM ciudadano WHERE ciudadano_id = $1', [employee.citizenId]);
        if (rowCount === 0) throw new RpcException({ statusCode: statusCode.BAD_REQUEST, message: 'Citizen not found' });
        finalCitizenId = employee.citizenId;
      } else {
        const rows = await client.query<any>('INSERT INTO ciudadano (ciudadano_id, nombres, apellidos, created_at, updated_at) VALUES (uuid_generate_v4(), $1, $2, NOW(), NOW()) RETURNING ciudadano_id', [employee.firstName, employee.lastName]);
      ...
      const employeeRows = await client.query<UserEmployeeSQLResult>(query, [employee.userId, finalCitizenId, employee.idCard, employee.firstName, employee.lastName, employee.positionId, employee.contractTypeId, employee.employeeStatusId]);
      return UserEmployeeAdapter.fromUserEmployeeSQLResultToUserEmployeeSQLResponse(employeeRows[0]);
    });
  }

  async update(employeeId: string, updates: Partial<UserEmployeeModel>): Promise<UserEmployeeResponse | null> {
    ...
    const result = await this.databaseService.query<UserEmployeeSQLResult>(query, [updates.firstName, updates.lastName, employeeId]);
    return result.length > 0 ? UserEmployeeAdapter.fromUserEmployeeSQLResultToUserEmployeeSQLResponse(result[0]) : null;
  }

  async softDelete(employeeId: string): Promise<void> {
    await this.databaseService.execute('UPDATE empleados SET deleted_at = NOW() WHERE empleado_id = $1', [employeeId]);
  }
  ...
  async assignZones(employeeId: string, zoneIds: number[]): Promise<void> {
    await this.databaseService.execute('UPDATE empleados SET zonas_asignadas = $1 WHERE empleado_id = $2', [JSON.stringify(zoneIds), employeeId]);
  }

  async changeStatus(employeeId: string, newStatusId: number): Promise<UserEmployeeResponse | null> {
    const query = `UPDATE empleados SET estado_empleado_id = $1 WHERE empleado_id = $2 RETURNING ${this.SELECT_ALL_FIELDS}`;
    const result = await this.databaseService.query<UserEmployeeSQLResult>(query, [newStatusId, employeeId]);
    return result.length > 0 ? UserEmployeeAdapter.fromUserEmployeeSQLResultToUserEmployeeSQLResponse(result[0]) : null;
  }

  async changeSupervisor(employeeId: string, supervisorId: string | null): Promise<UserEmployeeResponse | null> {
    const query = `UPDATE empleados SET supervisor_id = $1 WHERE empleado_id = $2 RETURNING ${this.SELECT_ALL_FIELDS}`;
    const result = await this.databaseService.query<UserEmployeeSQLResult>(query, [supervisorId, employeeId]);
    return result.length > 0 ? UserEmployeeAdapter.fromUserEmployeeSQLResultToUserEmployeeSQLResponse(result[0]) : null;
  }
  ...
  async findAllActive(): Promise<UserEmployeeResponse[]> {
    ...
    const result = await this.databaseService.query<UserEmployeeSQLResult>(query);
    return result.map(row => UserEmployeeAdapter.fromUserEmployeeSQLResultToUserEmployeeSQLResponse(row));
  }
  ...
  async findByPosition(positionId: number): Promise<UserEmployeeResponse[]> {
    ...
    const result = await this.databaseService.query<UserEmployeeSQLResult>(query, [positionId]);
    return result.map(row => UserEmployeeAdapter.fromUserEmployeeSQLResultToUserEmployeeSQLResponse(row));
  }
  ...
  async findByZone(zoneId: number): Promise<UserEmployeeResponse[]> {
    ...
    const result = await this.databaseService.query<UserEmployeeSQLResult>(query, [JSON.stringify([zoneId])]);
    return result.map(row => UserEmployeeAdapter.fromUserEmployeeSQLResultToUserEmployeeSQLResponse(row));
  }

  async findBySupervisor(supervisorId: string): Promise<UserEmployeeResponse[]> {
    const query = `SELECT ${this.SELECT_ALL_FIELDS} FROM empleados WHERE supervisor_id = $1 AND deleted_at IS NULL;`;
    const result = await this.databaseService.query<UserEmployeeSQLResult>(query, [supervisorId]);
    return result.map(row => UserEmployeeAdapter.fromUserEmployeeSQLResultToUserEmployeeSQLResponse(row));
  }

  async findAllEmployees(limit: number, offset: number): Promise<UserEmployeeResponse[]> {
    const query = `SELECT ${this.SELECT_ALL_FIELDS} FROM empleados WHERE deleted_at IS NULL LIMIT $1 OFFSET $2;`;
    const result = await this.databaseService.query<UserEmployeeSQLResult>(query, [limit, offset]);
    return result.map(row => UserEmployeeAdapter.fromUserEmployeeSQLResultToUserEmployeeSQLResponse(row));
  }
}
